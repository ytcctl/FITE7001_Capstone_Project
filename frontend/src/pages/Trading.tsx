...
const Trading: React.FC = () => {
  ...
  // Admin status (for force-cancel)
  const [isAdmin, setIsAdmin] = useState(false);
  const [forceCancelling, setForceCancelling] = useState<bigint | null>(null);
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      if (!obContract || !account) { setIsAdmin(false); return; }
      try {
        const adminRole = await obContract.DEFAULT_ADMIN_ROLE();
        const has = await obContract.hasRole(adminRole, account);
        setIsAdmin(has);
  ...
  // ---------------------------------------------------------------------------
  // Force-cancel a single order (Admin)
  // ---------------------------------------------------------------------------

  const handleForceCancel = async (orderId: bigint) => {
    if (!obContract) return;
    setForceCancelling(orderId);
    try {
      const tx = await obContract.forceCancelOrder(orderId, 'Admin force-cancel');
      await tx.wait();
      fetchData();
    } catch (err) {
  ...
  // ---------------------------------------------------------------------------
  // Cancel all orders for a non-compliant investor (Admin)
  // ---------------------------------------------------------------------------

  const handleCancelNonCompliant = async () => {
    if (!obContract || !forceCancelAddr.trim()) return;
    setForceCancelStatus('');
    try {
      const addr = ethers.getAddress(forceCancelAddr.trim());
      const tx = await obContract.cancelOrdersForNonCompliant(addr);
      await tx.wait();
      setForceCancelStatus(`✓ All open orders for ${addr.slice(0, 6)}…${addr.slice(-4)} cancelled`);
      setForceCancelAddr('');
      fetchData();
    } catch (err: any) {
      const msg = err?.reason || err?.message || 'Unknown error';
      setForceCancelStatus(`✗ ${msg}`);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {selectedMarket && (
        <>
          ...
          {/* ── Admin: Compliance Force-Cancel ── */}
          {isAdmin && (
            <div className="bg-red-950/30 rounded-2xl border border-red-500/30 p-6">
              <h2 className="text-lg font-semibold text-red-400 mb-4 flex items-center gap-2">
                <ShieldAlert size={20} />
                Compliance — Force Cancel Orders
              </h2>
              <p className="text-gray-400 text-sm mb-4">
                Cancel all open orders for an investor whose KYC/PEP status has been revoked.
              </p>
              <div className="flex gap-3 items-end">
                ...
                <button
                  onClick={handleCancelNonCompliant}
                  disabled={!forceCancelAddr.trim()}
                  className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-semibold text-sm disabled:opacity-40 transition-colors whitespace-nowrap"
                >
                  Cancel All Orders
                </button>
              </div>
              {forceCancelStatus && (
                <p className={`mt-3 text-sm ${forceCancelStatus.startsWith('✓') ? 'text-green-400' : 'text-red-400'}`}>
                  {forceCancelStatus}
                </p>
              )}

              {/* Admin can also force-cancel individual orders from the full book */}
              {(buyOrders.length > 0 || sellOrders.length > 0) && (
                <div className="mt-6">
                  <h3 className="text-sm font-semibold text-gray-400 mb-2">All Open Orders (admin force-cancel)</h3>
                  <div className="overflow-x-auto max-h-60 overflow-y-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-gray-500 text-xs border-b border-white/10">
                          <th className="text-left py-1 px-2">ID</th>
                          <th className="text-left py-1 px-2">Trader</th>
                      ...
                      <tbody>
                        {[...buyOrders, ...sellOrders]
                          .filter(o => Number(o.status) === OrderStatus.Open || Number(o.status) === OrderStatus.PartiallyFilled)
                          .map((o, i) => (
                            <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                              <td className="py-1 px-2 text-gray-300 font-mono">#{Number(o.id)}</td>
                              <td className="py-1 px-2 text-gray-400 font-mono text-xs">{o.trader.slice(0, 6)}…{o.trader.slice(-4)}</td>
                              <td className="py-1 px-2">
                                <span className={Number(o.side) === Side.Buy ? 'text-green-400' : 'text-red-400'}>
                                  {Number(o.side) === Side.Buy ? 'BUY' : 'SELL'}
                                </span>
                              </td>
                              <td className="py-1 px-2 text-right text-white font-mono">{fmtPrice(o.price)}</td>
                              <td className="py-1 px-2 text-right text-gray-300 font-mono">{fmtSec(o.quantity - o.filled)}</td>
                              <td className="py-1 px-2 text-center">
                                <button
                                  onClick={() => handleForceCancel(o.id)}
                                  disabled={forceCancelling === o.id}
                                  className="text-red-400 hover:text-red-300 text-xs font-semibold disabled:opacity-50"
                                >
                                  {forceCancelling === o.id ? <Loader2 size={12} className="animate-spin inline" /> : 'Force Cancel'}
                                </button>
                              </td>
                            </tr>
                          ))}
